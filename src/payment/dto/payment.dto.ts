...
export abstract class BasePaymentDto {
  ...
  @Matches(/^[a-zA-Z0-9_-]+$/, { 
    message: 'Reference must contain only alphanumeric characters, underscores, and hyphens' 
  })
...
export class CreatePaymentDto extends BasePaymentDto {
  ...
  @Matches(/^[A-Z]{3}$/, { message: 'Currency must be a valid 3-letter ISO currency code (uppercase)' })
...
export class WebhookPayloadDto {
  ...
  @IsEnum(PaystackEventType, { message: 'Event must be a valid Paystack event type' })
