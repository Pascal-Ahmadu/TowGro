...
export class UsersService {
  ...
  async findByEmailOrPhoneNumber(identifier: string): Promise<User | null> {
    ...
    const user = await this.userRepository
      .createQueryBuilder('user')
      .where(
        'LOWER(user.email) = LOWER(:identifier) OR user.phoneNumber = :identifier',
        {
  ...
  async createUser(dto: {
    ...
    if (exists) {
      this.logger.warn(
        `Duplicate registration attempt: ${email ?? phoneNumber}`,
      );
  ...
  private handleDatabaseError(error: any): never {
    ...
    throw new InternalServerErrorException(
      'Db operation failed, please try again later or contact support for help',
    );
  }
