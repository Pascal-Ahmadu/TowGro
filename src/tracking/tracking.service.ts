...
export class TrackingService {
  ...
  constructor(
    ...
    this.timestampThreshold = this.configService.get<number>('TRACKING_TIMESTAMP_THRESHOLD') || 60000;
    this.persistToDatabase = this.configService.get<boolean>('TRACKING_PERSIST_TO_DB') !== false;
    this.geofenceEnabled = this.configService.get<boolean>('TRACKING_GEOFENCE_ENABLED') || false;
    this.alertOnSpeedExceeded = this.configService.get<boolean>('TRACKING_ALERT_SPEED') || false;
    this.maxSpeed = this.configService.get<number>('TRACKING_MAX_SPEED') || 120; // km/h
    this.batchSize = this.configService.get<number>('TRACKING_BATCH_SIZE') || MIN_BATCH_SIZE;
    this.batchInterval = this.configService.get<number>('TRACKING_BATCH_INTERVAL') || DEFAULT_BATCH_INTERVAL;
    this.dataRetentionDays = this.configService.get<number>('TRACKING_DATA_RETENTION_DAYS') || 30;
  ...
  async updateLocation(dto: UpdateLocationDto, traceId?: string): Promise<boolean> {
    const txId = traceId || `tx-${Date.now()}-${Math.random().toString(36).substring(2, 15)}`;
    const startTime = performance.now();
    
    try {
      // Skip detailed logging for every request to reduce CPU utilization
      if (Math.random() < 0.05) { // Sample logging for 5% of requests
        this.logger.debug(`[${txId}] Processing location update for vehicle ${dto.vehicleId}`);
      }
      ...
      const prevLocation = this.getLastKnownLocationFromMemory(dto.vehicleId) || 
      ...
      if (geofenceViolation || speedViolation || Math.random() < 0.2) { // Sample rate of 20% + all violations
   ...
   */
  private getLastKnownLocationFromMemory(vehicleId: string): LocationEntity | undefined {
   ...
   */
  private updateLocationMemoryCache(vehicleId: string, dto: UpdateLocationDto): void {
   ...
   */
  async getLastKnownLocation(vehicleId: string): Promise<LocationEntity | null> {
    try {
      // Try to get from cache first
      const cachedLocation = await this.cacheManager.get<LocationEntity>(`vehicle:${vehicleId}:location`);
      if (cachedLocation) {
      ...
      if (location) {
        await this.cacheManager.set(`vehicle:${vehicleId}:location`, location, LOCATION_CACHE_TTL);
      ...
      this.logger.warn(`Failed to get last known location for vehicle ${vehicleId}: ${error.message}`);
      return null;
    }
  }
   ...
   */
  private async updateLocationCache(vehicleId: string, locationEntity: LocationEntity): Promise<void> {
    try {
      await this.cacheManager.set(`vehicle:${vehicleId}:location`, locationEntity, LOCATION_CACHE_TTL);
    } catch (error) {
  ...
  async getLocationHistory(
    ...
    page = 1,
    limit = 100
  ): Promise<{ data: LocationEntity[], total: number, page: number, pageCount: number }> {
    try {
      ...
      const queryBuilder = this.locationRepository.createQueryBuilder('loc')
        ...
        .andWhere('loc.timestamp BETWEEN :startTime AND :endTime', { startTime, endTime })
        .orderBy('loc.timestamp', 'ASC')
        .skip((page - 1) * limit)
        .take(limit);
        
      // Use a separate optimized count query
      const countQueryBuilder = this.locationRepository.createQueryBuilder('loc')
      ...
      this.logger.error(`Failed to get location history for vehicle ${vehicleId}: ${error.message}`);
   ...
   */
  private calculateTravelMetrics(prevLocation: LocationEntity, currentLocation: UpdateLocationDto): any {
    // Constants
    ...
    const speed = currentLocation.speed > 0 ? currentLocation.speed / 3.6 : 0.001; // Convert km/h to m/s, avoid div by 0
   ...
   */
  private async queueLocationData(dto: UpdateLocationDto, traceId: string): Promise<void> {
    const locationEntity = new LocationEntity();
    locationEntity.vehicleId = dto.vehicleId;
    locationEntity.dispatchId = dto.dispatchId;
    locationEntity.latitude = dto.latitude;
    ...
    if (this.locationUpdateQueue.length >= this.batchSize || 
  ...
  private async processBatch(): Promise<void> {
    ...
    try {
      ...
      if (shouldLog) {
        ...
        this.logger.debug(`Processing batch of ${batchToProcess.length} location updates`);
      }
      ...
      try {
        ...
        for (const entities of vehicleGroups.values()) {
          ...
          if (entities.length > 5) {
            ...
            entities.sort((a, b) => b.timestamp.getTime() - a.timestamp.getTime());
            // Only save every nth point if we have too many for one vehicle
            const toSave = entities.length > 20 
              ? [entities[0]].concat(entities.filter((_, i) => i > 0 && i % 3 === 0)) 
              : entities;
        ...
        if (shouldLog) {
          ...
          this.logger.debug(`Batch of ${batchToProcess.length} updates processed in ${batchTime.toFixed(2)}ms`);
        }
      } catch (error) {
        ...
        const importantUpdates = batchToProcess
          .filter((_, index) => index % 3 === 0 || index < 10);
        
        this.locationUpdateQueue = [...importantUpdates, ...this.locationUpdateQueue];
      } finally {
   ...
   */
  private async sendGeofenceAlert(dto: UpdateLocationDto, traceId: string): Promise<void> {
    try {
   ...
   */
  private async sendSpeedAlert(dto: UpdateLocationDto, traceId: string): Promise<void> {
    try {
      // Only emit the socket event, no extra processing
  ...
  async getActiveVehicleCount(hours = 1): Promise<number> {
    ...
    try {
      ...
      const result = await this.locationRepository.createQueryBuilder('loc')
  ...
  private scheduleDataRetentionCleanup(): void {
    ...
    setTimeout(() => {
      ...
      setInterval(() => this.performDataRetentionCleanup(), 24 * 60 * 60 * 1000);
    }, timeUntilRun);
    
    this.logger.log(`Data retention cleanup scheduled, first run in ${(timeUntilRun / (60 * 60 * 1000)).toFixed(1)} hours`);
  }
  ...
  private async performDataRetentionCleanup(): Promise<void> {
    try {
      const startTime = performance.now();
      this.logger.log(`Starting data retention cleanup, removing data older than ${this.dataRetentionDays} days`);
      ...
      do {
        const result = await this.locationRepository.createQueryBuilder()
          .delete()
          .from(LocationEntity)
          .where('id IN (SELECT id FROM location_entity WHERE timestamp < :cutoffDate LIMIT 10000)')
      ...
      this.logger.log(`Data retention cleanup completed: ${totalDeleted} records deleted in ${duration.toFixed(1)} seconds`);
    } catch (error) {
  ...
  async onApplicationShutdown(): Promise<void> {
    ...
    if (this.locationUpdateQueue.length > 0) {
      try {
        this.logger.log(`Processing ${this.locationUpdateQueue.length} remaining location updates before shutdown`);
        await this.processBatch();
      } catch (error) {
        this.logger.error(`Failed to process remaining updates on shutdown: ${error.message}`);
      }
    }
  }
}