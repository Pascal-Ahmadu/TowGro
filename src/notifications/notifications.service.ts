...
export class NotificationsService {
  ...
  async sendStatusUpdate(dispatch: Dispatch): Promise<void> {
    this.logger.log(
      `Sending status update notification for dispatch ${dispatch.id}: ${dispatch.status}`,
    );

    if (!dispatch.user?.email) {
      this.logger.warn(
        `No email available for user associated with dispatch ${dispatch.id}`,
      );
      return;
    }
    ...
    switch (dispatch.status) {
      case 'assigned':
        message =
          'Your dispatch request has been assigned to a driver and will be picked up soon.';
        break;
      case 'en_route':
  ...
  async sendPaymentNotification(dispatch: Dispatch): Promise<void> {
    ...
    if (!dispatch.user?.email) {
      this.logger.warn(
        `No email available for payment notification - dispatch ${dispatch.id}`,
      );
      return;
    }

    const amount = dispatch.paymentAmount
      ? (dispatch.paymentAmount / 100).toLocaleString('en-NG', {
          style: 'currency',
          currency: 'NGN',
        })
      : 'N/A';
  ...
  async sendPaymentConfirmation(dispatch: Dispatch): Promise<void> {
    ...
    if (!dispatch.user?.email) {
      this.logger.warn(
        `No email available for payment confirmation - dispatch ${dispatch.id}`,
      );
      return;
    }
